...
public class FriendsActivity extends FragmentActivity implements OnMapReadyCallback, LocationListener {
    ...
    private class FriendFinderTask extends AsyncTask<String, Integer, List<users>> {
        ...
        protected List<users> doInBackground(String... params) {
            URL url;
            String response = "";
            String REQUEST_URL = BASE_URL + "find_frnds.php";
        ...
        protected void onPostExecute(List<users> friends_nearby) {
            ...
            for (users friend : friends_nearby){
                ...
                my_Location = new LatLng(friend.getLatitude(), friend.getLongitude());
                if (mMap != null) {
                    mMap.addMarker(new MarkerOptions().position(my_Location).title(friend.getFull_name()).icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_YELLOW))).showInfoWindow();
                }
            }
            my_Location = myLocation;
            if (myLocation!= null && myLocation.latitude!= 0) {
                mMap.addMarker(new MarkerOptions().position(my_Location).title("my location").icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_AZURE))).showInfoWindow();
                if(!flag) {
                    CameraPosition cp = new CameraPosition.Builder().target(my_Location).zoom(15).build();
